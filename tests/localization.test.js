...
describe("localized strings used by main.js", () => {
  ...
  it.each([
    "STARDICEBAR.Mode.Normal",
    "STARDICEBAR.Mode.Advantage",
    "STARDICEBAR.Mode.Disadvantage",
    "STARDICEBAR.Bar.EmptyHint",
    "STARDICEBAR.Flavor.QuickRoll",
    "STARDICEBAR.Tab.Dice",
    "STARDICEBAR.Notify.InvalidFormula",
    "STARDICEBAR.Settings.HideBar.Name",
  ])("defines %s", (key) => {
    expect(en[key]).toBeTruthy();
  });

  it("QuickRoll carries a {dice} placeholder for interpolation", () => {
    expect(en["STARDICEBAR.Flavor.QuickRoll"]).toContain("{dice}");
  });

  it("AlreadyExists carries a {die} placeholder for interpolation", () => {
    expect(en["STARDICEBAR.Notify.AlreadyExists"]).toContain("{die}");
  });
});
